...
function state_space!(F, x, meas, b::CirculatingCurrentSuppressionControl, conv::AbstractMMC) 
    ...
    # vMΣd_ref = 2/Vdc*(- Ki_Σ * xiΣ_d - Kp_Σ * (iΣ_d_ref -  iΣ_d) + 2*Larm*iΣ_q)
    # vMΣq_ref = 2/Vdc*(- Ki_Σ * xiΣ_q - Kp_Σ * (iΣ_q_ref -  iΣ_q) - 2*Larm*iΣ_d)
    # Assuming constant w
    vMΣd_ref_c = (- ξ_iΣ_d - b.pi_control.Kp * (iΣ_d_ref - iΣ_d_c) + 2 * conv.elec.Lₐᵣₘ * iΣ_q_c)
    vMΣq_ref_c = (- ξ_iΣ_q - b.pi_control.Kp * (iΣ_q_ref - iΣ_q_c) - 2 * conv.elec.Lₐᵣₘ * iΣ_d_c)
...
function state_space!(F, x, (meas, out_Wtot), b::ZeroSequenceCurrentControl, conv::AbstractMMC)
    ...
    return ( vMΣz_ref = (v_dc_f/2 - b.pi_control.Kp *(iΣ_z_ref - iΣ_z) - ξ_iΣ_z), )
end
