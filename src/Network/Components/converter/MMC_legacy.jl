...
function state_space!(F,x,inputs;converter, solver)
    init_x = zeros(12, 1) # Initial steady-state values for MMC model
...
########################################################Linearization###################################################################
    h(F,x) = state_space!(F, x[1:end-3], x[end-2:end];converter, solver=:jacobian) # Using callable function from above 
