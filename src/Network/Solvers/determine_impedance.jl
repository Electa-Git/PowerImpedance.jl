...
function determine_impedance(network::Network; input_pins::Array{Symbol},
    ...
    function make_lists(net::Network, dict::Dict{Symbol, Array{Union{Symbol, Int}}},
        ...
        for node_name in start_pins
            ...
            elements_pins = filter(
                p -> !in(p[1], elim_elements) &&
                     !in(p[1], dict[:element_list]), node)

            for (element, pin) in elements_pins
                (!in(element, dict[:element_list]) && net.elements[element].connection) &&
                    push!(dict[:element_list], element) # add element's symbol to the list, only if the element has not been added before
    ...
    isempty(input_pins) &&
        throw(ArgumentError("Impedance cannot be determined from nonexistent input port."))
    isempty(output_pins) &&
        throw(ArgumentError("Impedance cannot be determined from nonexistent output port."))
    ...
    dict = Dict{Symbol, Array{Union{Symbol, Int}}}(
        :node_list => Symbol[], :element_list => Symbol[],
        :output_list => Symbol[])
    ...
    if any(!in(end_pin, dict[:output_list]) for end_pin in output_pins)
        dict_o = Dict{Symbol, Array{Union{Symbol, Int}}}(
            :node_list => Symbol[], :element_list => Symbol[],
            :output_list => Symbol[])
    ...
    dict[:node_list] = dict[:node_list][[input_order;
                                         setdiff(1:length(dict[:node_list]), input_order)]]
    for output in unique(output_pins)
        i = findfirst(p -> p == output, dict[:output_list])
        push!(output_order, i)
    end
    dict[:output_list] = dict[:output_list][[output_order;
                                             setdiff(1:length(dict[:output_list]), output_order)]]
