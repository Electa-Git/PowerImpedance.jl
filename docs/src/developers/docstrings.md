...
Document explicitly defined constructors with `@doc`, including compact assignment forms. Use the standard section order and document the constructed object:
...
6. `# Examples`.
7. `# See also`, only for meaningful package-local references.
...
Examples must use meaningful input values. Include an expected result or behavior only when it can be stated accurately. Link primarily to related functions in this package.
...
- Inferring mathematical content from a `calc_` prefix or any other naming pattern.
...
- Omitting `@doc` from an explicitly defined constructor.
