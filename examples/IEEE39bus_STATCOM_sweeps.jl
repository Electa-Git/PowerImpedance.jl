...
@time begin
struct ModeInfo
# A container to hold the mode information derived with positive net damping criterion 
...
function IEEE39bus_powerflow!(grid::Network,control_param:: Float64, Q_ST :: Float64, Vac_ref_ST :: Float64)
# Q_ST [SI/MVA], Vac_ref_ST [SI/kV]
        ...
        grid.elements[Symbol("STATCOM")].element_value.controls[:vac_supp].Kₚ=control_param
...
max_f=150 # Maximum frequency in Hz 
n_f=300 # Number of frequency points in the
...
param_range=range(start=15.0, stop=0.1*15.0, step=-0.1*15.0) # Parameter variation range 

# Arrange storage for the results 
# Grid impedances can be taken from the previous IEEE39bus topology runs

# Debug dict
GNC_result=nothing;
powerflow_result=nothing


@time for (k,param) in enumerate(param_range)
       
        Y_Statcom_ac = Array{Matrix{ComplexF64},3}(undef, length(Q_steps), length(V_steps), n_f) # Q-V-param-f-STATCOM_admittance_ac
        GNCs=Array{GNCInfo}(undef, length(Q_steps), length(V_steps)) # Q-V-GNCs
        powerflow=Array{PowerFlowResult}(undef, length(Q_steps), length(V_steps)) # Q-V-Powerflow results
        topology[:T9_39]=false 
        grid=IEEE39bus_parametric(topology)
        Zgrid_T939, omega=determine_impedance(grid, input_pins=[:Bus9d,:Bus9q], output_pins=[:gndD, :gndQ], elim_elements=[:STATCOM],freq_range=(min_f, max_f,n_f))
        topology[:T9_39]=true
        ...
        for (i, Q) in enumerate(Q_steps) # Iterate over all Q-setpoints
                ...
                for (j, V) in enumerate(V_steps) # Iterate over all V-setpoints
                        ...
                        Vm_pf,Va_pf,P_pf,Q_pf,Vdc_pf,Pdc_pf,Q_fixed=@time IEEE39bus_powerflow!(grid,param,Q*269,V*sqrt(2/3)*345) # Update power flow and obtain STATCOM Y for the given operating point
                        powerflow[i,j] =PowerFlowResult(Vm_pf,Va_pf,P_pf,Q_pf,Vdc_pf,Pdc_pf,Q_fixed)
                        for (l, ω) in enumerate(omega)
                                Y1 = PowerImpedanceACDC.eval_abcd(grid.elements[:STATCOM].element_value,ω*1im)
                                Y_Statcom= [transpose(Y1[1,:]); transpose(-Y1[2,:]); transpose(-Y1[3,:])]
                                #Do collapsing of the DC-side with DC cap
                                Y11=Y_Statcom[1,1]
                                Y12=Y_Statcom[1,2]
                                Y13=Y_Statcom[1,3]
                                Y21=Y_Statcom[2,1]
                                ...
                                Y22₊=(Y21*Y12+(-Yc-Y11)*Y22)/(-Yc-Y11)
                                Y23₊=(Y21*Y13+(-Yc-Y11)*Y23)/(-Yc-Y11)
                                Y32₊=(Y31*Y12+(-Yc-Y11)*Y32)/(-Yc-Y11)
                                Y33₊=(Y31*Y13+(-Yc-Y11)*Y33)/(-Yc-Y11)
                                Y_Statcom_ac[i, j, l]= [Y22₊ Y23₊; Y32₊ Y33₊] # Collapsed 2x2 admittance matrix for the STATCOM at line i, Q-step j, V-step k, and frequency l
        ...
        # After data aggregation do the analysis for the given parameter, ie. check whether vector margin is obtained :) 
        for i in 1:length(Q_steps)  # Q_steps
                for j in 1:length(V_steps)  # V_steps
                        loopgain=Zgrid_T939.*Y_Statcom_ac[i, j, :] 
                        stability, gain_margins, frequencies_GM, phase_margins, frequencies_PM, vector_margins, frequencies_VM = Nyquist(loopgain, omega)
                        GNCs[i, j] = GNCInfo(stability, gain_margins[1], frequencies_GM[1], phase_margins[1],frequencies_PM[1], vector_margins[1], frequencies_VM[1], gain_margins[2], frequencies_GM[2], phase_margins[2], frequencies_PM[2], vector_margins[2], frequencies_VM[2])

                end
        end
        # Double-check vector margins for all cases :)
        #Check stability first
        stability_stopper=false
        for i in eachindex(Q_steps)
                for j in eachindex(V_steps)
                        if GNCs[i, j].stability !=1
                                stability_stopper=true
                                break
                        end
                end
                stability_stopper && break
        end
        stability_stopper && continue # If unstable, skip to the next parameter value without saving the results

        # Check vector margins
        vm_stopper=false
        for i in eachindex(Q_steps)
                for j in eachindex(V_steps)
                        if GNCs[i, j].VM1[1] < 10.0 || GNCs[i, j].VM2[1] < 10.0
                                vm_stopper=true
                                break
                        end
                end
        ...
        vm_stopper && continue # If vector margin is negative, skip to the next parameter value without saving the results


        if !vm_stopper # VM fullfilled for T9_39, double-check whether this is also the case for the other lines
                # Double-check whether all the other cases for the found parameter give sufficient VM as well
                Y_Statcom_ac_grid = Array{Matrix{ComplexF64},4}(undef, length(topology), length(Q_steps), length(V_steps), n_f) # Line-Q-V-param-f-STATCOM_admittance_ac
                GNCs_grid=Array{GNCInfo}(undef,length(topology), length(Q_steps), length(V_steps)) # Line-Q-V-GNCs
                powerflow_grid=Array{PowerFlowResult}(undef,length(topology), length(Q_steps), length(V_steps)) # Q-V-Powerflow results
                # Again Zgrid can be reused
                Zgrid=Array{Matrix{ComplexF64},2}(undef,length(topology), n_f) # line-f-AC_impedance # Can be improved as Zgrid has been stored before already :)
                for lines in keys(topology)
                        ...
                        grid=IEEE39bus_parametric(topology)
                        # Can be streamlined as Zgrid has already been obtained 
                        @time Zgrid[index, :], omega=determine_impedance(grid, input_pins=[:Bus9d,:Bus9q], output_pins=[:gndD, :gndQ], elim_elements=[:STATCOM],freq_range=(min_f, max_f,n_f))
                        ...
                        for (i,Q) in enumerate(Q_steps) # Iterate over all Q-setpoints
                                ...
                                for (j,V) in enumerate(V_steps) # Iterate over all V-setpoints
                                        ...
                                        Vm_pf,Va_pf,P_pf,Q_pf,Vdc_pf,Pdc_pf,Q_fixed=@time IEEE39bus_powerflow!(grid,param,Q*269,V*sqrt(2/3)*345) # Update power flow and obtain STATCOM Y for the given operating point
                                        powerflow_grid[index,i,j] =PowerFlowResult(Vm_pf,Va_pf,P_pf,Q_pf,Vdc_pf,Pdc_pf,Q_fixed)
                                        for (l, ω) in enumerate(omega)
                                                Y1 = PowerImpedanceACDC.eval_abcd(grid.elements[:STATCOM].element_value,ω*1im)
                                                Y_Statcom= [transpose(Y1[1,:]); transpose(-Y1[2,:]); transpose(-Y1[3,:])]
                                                #Do collapsing of the DC-side with DC cap
                                                ...
                                                Y32₊=(Y31*Y12+(-Yc-Y11)*Y32)/(-Yc-Y11)
                                                Y33₊=(Y31*Y13+(-Yc-Y11)*Y33)/(-Yc-Y11)
                                                Y_Statcom_ac_grid[index, i, j, l]= [Y22₊ Y23₊; Y32₊ Y33₊] # Collapsed 2x2 admittance matrix for the STATCOM at line i, Q-step j, V-step k, and frequency l
                        ...
                        topology[lines]=true # Reconnect the line for the next iteration




                end
                # Check vector margins for all cases with the found parameter value
                for lines in keys(topology)
                        if lines == :T9_39
                                continue
                        end
                        ...
                        for i in 1:length(Q_steps)  # Q_steps
                                for j in 1:length(V_steps)  # V_steps
                                        loopgain=Zgrid[index,:].*Y_Statcom_ac_grid[index,i, j, :] # Loop gain = Zgrid * Y_Statcom(2:3, 2:3)
                                        stability, gain_margins, frequencies_GM, phase_margins, frequencies_PM, vector_margins, frequencies_VM = Nyquist(loopgain, omega)
                                        GNCs_grid[index,i, j] = GNCInfo(stability, gain_margins[1], frequencies_GM[1], phase_margins[1],frequencies_PM[1], vector_margins[1], frequencies_VM[1], gain_margins[2], frequencies_GM[2], phase_margins[2], frequencies_PM[2], vector_margins[2], frequencies_VM[2])
                ...
                for lines in keys(topology)
                        ...
                        for i in eachindex(Q_steps)
                                for j in eachindex(V_steps)
                                        if GNCs_grid[index,i, j].stability !=1
                                                stability_stopper_grid=true
                                                break
                                        end
                                end
                                stability_stopper_grid && break
                        end
                        stability_stopper_grid && break
                end
                stability_stopper_grid && continue # If unstable, skip to the next parameter value without saving the results

                # Check vector margins
                vm_stopper_grid=false
                for lines in keys(topology)
                        if lines == :T9_39
                                continue
                        end
                        ...
                        for i in eachindex(Q_steps)
                                for j in eachindex(V_steps)
                                        if GNCs_grid[index,i, j].VM1[1] < 10.0 || GNCs_grid[index,i, j].VM2[1] < 10.0
                                                vm_stopper_grid=true
                                                break
                                        end
                                end
                                vm_stopper_grid && break
                        end
                        vm_stopper_grid && break
                end
                vm_stopper_grid && continue # If vector margin is negative, skip to the next parameter value without saving the results
                


                # Build complete results
                GNCs_grid[findfirst(==(:T9_39),collect(keys(topology))),:,:]=GNCs
                powerflow_grid[findfirst(==(:T9_39),collect(keys(topology))),:,:]=powerflow

                global GNC_result = GNCs_grid
                global powerflow_result = powerflow_grid
                println("Parameter value $(param) is stable with sufficient vector margin. Saving results...")
                break
        end
