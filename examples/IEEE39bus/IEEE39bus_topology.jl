...
function IEEE39bus_powerflow!(grid::Network,Q_ST :: Float64, Vac_ref_ST :: Float64,debug::Dict{Symbol, Tuple{Float64,Float64,Float64,Float64,Float64,Float64}},line::Symbol)
# Q_ST [SI/MVA], Vac_ref_ST [SI/kV]
        ...
        result, data, nodes2bus, elem2comp = PowerImpedanceACDC.power_flow(grid, debug=debug, line=line, Q_ST=Q_ST, Vac_ref_ST=Vac_ref_ST) # run power flow routine, which also obtains the linearized state-space of the STATCOM
        ...
        if abs((q_pf*result["solution"]["baseMVA"]))>grid.elements[Symbol("STATCOM")].element_value.Sbase
                ...
                result, data, nodes2bus, elem2comp = PowerImpedanceACDC.power_flow(grid, debug=debug, line=line, Q_ST=Q_ST, Vac_ref_ST=Vac_ref_ST)
        ...
        Vdc = result["solution"]["busdc"]["1"]["vm"] * (data["dcpol"] * data["busdc"]["1"]["basekVdc"])
...
Zg30=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg31=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg32=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg33=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg34=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg35=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg36=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg37=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg38=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
Zg39=impedance(z = (s::Complex)-> (0.191736  + s*0.0122063), pins = 3, transformation = true)
...
STATCOM= tlc(Vᵈᶜ = Vdc_ST, Lᵣ = Lf_ST, Rᵣ = Rf_ST, 
        ...
        vac_supp = PI_control(Kₚ=5,ω_f=1/0.5,ref=[Vm1*sqrt(2)*Vac_ref_ST]), 
        timeDelay = 150e-6,
        padeOrderNum = 3,                    
        padeOrderDen = 3 
)
...
n_f=1000 # Number of frequency points in the
...
# Q_steps=[-1.0, -0.5, 0.0, 0.5, 1.0]
# V_steps=[0.9,0.95, 1.0, 1.05, 1.1]

# Arrange storage system sir!

Zgrid = Array{Matrix{ComplexF64},2}(undef, length(topology), n_f) # Line-f-AC_impedance
Y_Statcom = Array{Matrix{ComplexF64},4}(undef, length(topology), length(Q_steps), length(V_steps), n_f) # Line-Q-V-f-STATCOM_admittance
...
powerflow=Array{PowerFlowResult}(undef, length(topology), length(Q_steps), length(V_steps)) # Line-Q-V-Powerflow results
# Debug dict
debug = Dict{Symbol, Tuple{Float64, Float64, Float64, Float64, Float64, Float64}}()


global rescue = false
@time for lines in keys(topology)
        ...
        @time dummy, omega=determine_impedance(grid, input_pins=[:Bus9d,:Bus9q], output_pins=[:gndD, :gndQ], elim_elements=[:STATCOM],freq_range=(min_f, max_f,n_f))
        index=findfirst(==(lines),collect(keys(topology)))
        Zgrid[index,:]=dummy


        for (i, Q) in enumerate(Q_steps) # Iterate over all Q-setpoints

                for (j, V) in enumerate(V_steps) # Iterate over all V-setpoints

                        Vm_pf,Va_pf,P_pf,Q_pf,Vdc_pf,Pdc_pf,Q_fixed=@time IEEE39bus_powerflow!(grid,Q*269,V*sqrt(2/3)*345, debug, lines) # Update power flow and obtain STATCOM Y for the given operating point
                        powerflow[index,i,j] =PowerFlowResult(Vm_pf,Va_pf,P_pf,Q_pf,Vdc_pf,Pdc_pf,Q_fixed)
                        
                        ######################## Power flow convergence check
                        # if !(Int(PowerImpedanceACDC.result["termination_status"]) == 4) && !(Int(PowerImpedanceACDC.result["termination_status"]) == 10)
                        #         println("Powerflow did not converge for line disconnection: $lines and Q-STATCOM=$Q & Vac-STATCOM=$V ")
                        ...
                        if (vm > 0.90) && (vm < 1.1)
                                ...
                                println("Powerflow converged for Q-STATCOM=$Q & Vac-STATCOM=$V in line disconnection: $lines with voltage magnitude at Bus9: $vm p.u.")
                        else
                                println("Powerflow converged for Q-STATCOM=$Q & Vac-STATCOM=$V in line disconnection: $lines but voltage magnitude at Bus9: $vm p.u. is out of limits.")
                                println("Breaking loop!")
                                global rescue = true
                                break
                        end
                        # Check whether all the other voltages are in limits as well
                        for (bus_name,bus_data) in PowerImpedanceACDC.result["solution"]["bus"]
                                vm_bus=bus_data["vm"]
                                if (vm_bus > 0.90) && (vm_bus < 1.1)

                                
                                else
                                        println("Bus $bus_name voltage magnitude $vm_bus p.u. is out of limits.")
                                        println("Breaking loop!")
                                        global rescue = true
                                        break
                        ...
                        for k in eachindex(omega)
                                Y1 = PowerImpedanceACDC.eval_abcd(grid.elements[:STATCOM].element_value, omega[k]*1im)
...
# Calculate Z_cl and apply EVD of Z_cl to obtain unstable modes
for i in 1:size(Y_Statcom, 1)  # lines
    for j in 1:size(Y_Statcom, 2)  # Q_steps
        for k in 1:size(Y_Statcom, 3)  # V_steps
            for l in 1:size(Y_Statcom, 4)  # frequencies
                
                Z_cl[i, j, k, l] = inv(inv(Zgrid[i, l]) + Y_Statcom[i, j, k, l][2:3, 2:3]) # Z_cl = inv(inv(Zgrid) + Y_Statcom)
...
# Calculate EVD of Z_cl
dummy,test=EVD_PND(Z_cl[1,1,1,:], omegas, min_f, max_f)

for i in 1:size(Y_Statcom, 1)  # lines
    for j in 1:size(Y_Statcom, 2)  # Q_steps
        for k in 1:size(Y_Statcom, 3)  # V_steps
               
                osc_f,damping=EVD_PND(Z_cl[i, j, k, :], omegas, min_f, max_f)
                modes[i, j, k]=ModeInfo(damping[1], damping[2], osc_f[1], osc_f[2])
...
plt=plot3d( 1,size = (600, 600),
    ...
    zlim = (0, 34),
    legend = false,
    xticks = ([-1.0, -0.5, 0.0, 0.5, 1.0], ["-1.0", "-0.5", "0", "0.5", "1.0"]),
    yticks = ([0.9,0.95, 1.0, 1.05, 1.1], ["0.9", "0.95", "1.0", "1.05", "1.1"]),
    #zticks = ([10, 30, 50, 70], ["10", "30", "50", "70"]),
    marker = 2,)


for i in 1:size(Y_Statcom, 1)  # lines
        for j in 1:length(Q_steps)  # Q_steps
                for k in 1:length(V_steps)  # V_steps

                marker = :square
                color = (minimum(modes[i,j,k].Reλ₁) < 0 || minimum(modes[i,j,k].Reλ₂) < 0 ) ? :red : :blue

                scatter3d!([Q_steps[j]], [V_steps[k]], [i];
                        markershape = marker,
                        markercolor = color,
                        markersize = 4,
                        xlabel = "Q [pu]",
...
println("Debug empty? Debug dict:debug = $(debug)")


# Separately investigate stability of the modes
global isstable = true
for i in 1:size(Y_Statcom, 1)  # lines
        for j in 1:length(Q_steps)  # Q_steps
                for k in 1:length(V_steps)  # V_steps

                        if minimum(modes[i,j,k].Reλ₁) < 0 || minimum(modes[i,j,k].Reλ₂) < 0 
                                
                                println("System is unstable for Line $(keys(topology)[i]) and for Q=$(Q_steps[j]) and V=$(V_steps[k])")
                                println("Unstable modes of modal impedance 1 are:")
                                println(modes[i,j,k].fλ₁[findall(x->x<0, modes[i,j,k].Reλ₁)])
                                println("Unstable modes of modal impedance 2 are:")
                                println(modes[i,j,k].fλ₂[findall(x->x<0, modes[i,j,k].Reλ₂)])
                                global isstable = false
                        end
...
println("Overall system stability: $(isstable)")



#####################################################################################################################Validation of results#############################################################################

# # Validation of "Zgrid" with PSCAD data for disconnection of T8_9 matching ✔️
# writedlm("imp_TL_validation.csv",  Zgrid[21,:], ',')
# writedlm("w_TL_validation.csv",  omegas, ',')

# # Loadflow validation "powerflow" ✔️✔️ matching for a couple of investigate cases

#Validation of Y_Statcom against PSCAD data for disconnection of T9_39

# adm=Y_Statcom[25,1,1,:] # For T9_39 disconnection, Q=1.0, V=1.1 --> Conversion to Q mode 
# θ=1*-0.084661059255593 # Powerflow angle from Ztool
# # Transformation from global DQ frame to local DQ frame of the Ztool scan!
# TdqDC_0 = [1 0 0; 0 cos(θ) +sin(θ); 0 -sin(θ) cos(θ)]
# TdqDC__0 = [1 0 0; 0 cos(-θ) +sin(-θ); 0 -sin(-θ) cos(-θ)]

# Ymmc = []
# for k in eachindex(omegas)

#         push!(Ymmc, TdqDC_0 *adm[k]* TdqDC__0)
...
# writedlm("./Y_MMC1.csv",  Ymmc, ',')
# writedlm("./omega.csv",  omegas, ',')

# # Ydd
# bode_Ydd = bodeplot(getindex.(Ymmc_correct,2,2), omegas,legend=["Ydd_loop"]) 
# plot!(bode_Ydd[1],omegas./(2*pi), 20*log10.(abs.(getindex.(adm,2,2))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydd_single"])# For different line styles: linestyle = :auto
# plot!(bode_Ydd[2],omegas./(2*pi), angle.(getindex.(adm,2,2)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydd_single"])

# display(bode_Ydd)


# # Ydq
# bode_Ydq = bodeplot(getindex.(Ymmc_correct,2,3), omegas,legend=["Ydq_loop"]) 
# plot!(bode_Ydq[1],omegas./(2*pi), 20*log10.(abs.(getindex.(adm,2,3))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydq_single"])# For different line styles: linestyle = :auto
# plot!(bode_Ydq[2],omegas./(2*pi), angle.(getindex.(adm,2,3)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydq_single"])

# display(bode_Ydq)


# # Yqd
# bode_Yqd = bodeplot(getindex.(Ymmc_correct,3,2), omegas,legend=["Yqd_loop"]) 
# plot!(bode_Yqd[1],omegas./(2*pi), 20*log10.(abs.(getindex.(adm,3,2))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqd_single"])# For different line styles: linestyle = :auto
# plot!(bode_Yqd[2],omegas./(2*pi), angle.(getindex.(adm,3,2)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqd_single"])

# display(bode_Yqd)

# # Yqq
# bode_Yqq = bodeplot(getindex.(Ymmc_correct,3,3), omegas,legend=["Yqq_loop"]) 
# plot!(bode_Yqq[1],omegas./(2*pi), 20*log10.(abs.(getindex.(adm,3,3))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqq_single"])# For different line styles: linestyle = :auto
# plot!(bode_Yqq[2],omegas./(2*pi), angle.(getindex.(adm,3,3)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqq_single"])
...
# Ymmc_single=[]
# # Compare with Y-data extract without loop from IEEE39bus_parametric(topology) with T9_39 disconnected Q=1.0 and Q mode
# # Compare again for STATCOM mode
# for k in eachindex(omegas)

#         dummy=eval_abcd(test.elements[:STATCOM].element_value,1im*omegas[k])
#         push!(Ymmc_single, [transpose(dummy[1,:]); transpose(-dummy[2,:]); transpose(-dummy[3,:])])

# end


# # Compare analytical vs. interpolated plots
# # Mismatch below 100 Hz!
# # Ydd
# bode_Ydd = bodeplot(getindex.(adm,2,2), omegas,legend=["Ydd_loop"]) 
# plot!(bode_Ydd[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,2,2))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydd_single"])# For different line styles: linestyle = :auto
# plot!(bode_Ydd[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,2,2)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydd_single"])

# display(bode_Ydd)


# # Ydq
# bode_Ydq = bodeplot(getindex.(adm,2,3), omegas,legend=["Ydq_loop"]) 
# plot!(bode_Ydq[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,2,3))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydq_single"])# For different line styles: linestyle = :auto
# plot!(bode_Ydq[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,2,3)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydq_single"])

# display(bode_Ydq)


# # Yqd
# bode_Yqd = bodeplot(getindex.(adm,3,2), omegas,legend=["Yqd_loop"]) 
# plot!(bode_Yqd[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,3,2))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqd_single"])# For different line styles: linestyle = :auto
# plot!(bode_Yqd[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,3,2)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqd_single"])

# display(bode_Yqd)

# # Yqq
# bode_Yqq = bodeplot(getindex.(adm,3,3), omegas,legend=["Yqq_loop"]) 
# plot!(bode_Yqq[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,3,3))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqq_single"])# For different line styles: linestyle = :auto
# plot!(bode_Yqq[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,3,3)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqq_single"])

# display(bode_Yqq)

######################################################################################################################################################
# Debug further
# Now with comparing STATCOM operation (Vac supp active) with Q_ac_ref = 0.5 and V_ac_ref = 1.0 p.u. 
# Calculation with IEEE39bus_parametric(topology) for T9_39 disconnection
# Calculation with IEEE39bus_powerflow!()
# In this case the admittances are matching! 

#Single
# test=IEEE39bus_parametric(topology)
# Ymmc_single=[]
# for k in eachindex(omegas)
...
# IEEE39bus_powerflow!(test,-.5*269,0.90*sqrt(2/3)*345, debug, :T9_39) 

# Ymmc=[]

# for k in eachindex(omegas)
#         Y1 = PowerImpedanceACDC.eval_abcd(test.elements[:STATCOM].element_value, omegas[k]*1im)
#         push!(Ymmc,[transpose(Y1[1,:]); transpose(-Y1[2,:]); transpose(-Y1[3,:])])
# end



# bode_Ydd = bodeplot(getindex.(Ymmc,2,2), omegas,legend=["Ydd_loop"]) 
# plot!(bode_Ydd[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,2,2))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydd_single"])# For different line styles: linestyle = :auto
# plot!(bode_Ydd[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,2,2)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydd_single"])
...
# # Ydq
# bode_Ydq = bodeplot(getindex.(Ymmc,2,3), omegas,legend=["Ydq_loop"]) 
# plot!(bode_Ydq[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,2,3))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydq_single"])# For different line styles: linestyle = :auto
# plot!(bode_Ydq[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,2,3)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Ydq_single"])
...
# # Yqd
# bode_Yqd = bodeplot(getindex.(Ymmc,3,2), omegas,legend=["Yqd_loop"]) 
# plot!(bode_Yqd[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,3,2))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqd_single"])# For different line styles: linestyle = :auto
# plot!(bode_Yqd[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,3,2)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqd_single"])
...
# # Yqq
# bode_Yqq = bodeplot(getindex.(Ymmc,3,3), omegas,legend=["Yqq_loop"]) 
# plot!(bode_Yqq[1],omegas./(2*pi), 20*log10.(abs.(getindex.(Ymmc_single,3,3))), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqq_single"])# For different line styles: linestyle = :auto
# plot!(bode_Yqq[2],omegas./(2*pi), angle.(getindex.(Ymmc_single,3,3)).*(180/π), linewidth = :auto, linestyle = :auto,minorgrid=true,label=["Yqq_single"])
...
# Try multiples cases to see whether the overwriting works :)

###########################################################################################################################################################
# Trying case now where a conversion from STATCOM mode to Q mode is happening!

#1. Calculation with IEEE39bus_parametric(topology) for T9_39 disconnection with Q=1.0 and V=1.1 p.u. with Q mode fixed in power flow
# to have the reference Y data
# test=IEEE39bus_parametric(topology)
...
# # 2. Call IEEE39bus_parametric(topology) again but with Vac support active and Q=1.0 and V=1.1 p.u

# test=IEEE39bus_parametric(topology)

# #3. Call IEEE39bus_powerflow!() 

# IEEE39bus_powerflow!(test,0.5*269,1.01*sqrt(2/3)*345, debug, :T9_39)
...
# # Result: Starting to see discrepancies between the loop and single calculations for conversion from Vac to Q mode!
...
# Save Y_Statcom data for 
# Q_steps=[1.0]
# V_steps=[1.1]
# using MAT
# Storage options
# file_name="02_10_2026_Julia_run3"
# internal_file_name="data"
# storage_path="C:/Users/jkirchei/OneDrive - KU Leuven/Documents/PhD Project/Work Package 2/Data/Raw/"


# dummy=Dict{String,Array{Matrix{ComplexF64},4}}()
# dummy["Y_Statcom"]=Y_Statcom
# matwrite(joinpath(storage_path, file_name * ".mat"), dummy; compress = false)
...
# using MAT
# Storage options
# file_name="02_10_2026_Julia_run4"
# internal_file_name="data"
# storage_path="C:/Users/jkirchei/OneDrive - KU Leuven/Documents/PhD Project/Work Package 2/Data/Raw/"
...
# matread("C:/Users/jkirchei/OneDrive - KU Leuven/Documents/PhD Project/Work Package 2/Data/Raw/02_10_2026_Julia_run4.mat")

# adm=dummy["Y_Statcom"][25,1,1,:]
...
#         push!(Ymmc, inv(TdqDC_0) *adm[k]* TdqDC_0)
...
###########################################################################################################Garbage code for testing and debugging purposes, to be removed later :)##########################################################################################################################


# # Sanity checking whether the admittance and impedance values are being stored correctly


# #:T4_14  => 0
# test=IEEE39bus_parametric(topology)
# IEEE39bus_powerflow!(test,-1.0*269,1.1*sqrt(2/3)*345)




# # Impedance check
# dummy, omegas=determine_impedance(test, input_pins=[:Bus9d,:Bus9q], output_pins=[:gndD, :gndQ], elim_elements=[:STATCOM],freq_range=(min_f, max_f,n_f))


# isequal(dummy,Zgrid[1]) # ✔️✔️

# # Admittance check ✔️✔️


# Y_Statcom_check=[]
# for k in eachindex(omegas)
# Y1 = PowerImpedanceACDC.eval_abcd(test.elements[:STATCOM].element_value, omegas[k]*1im)
# push!(Y_Statcom_check, [transpose(Y1[1,:]); transpose(-Y1[2,:]); transpose(-Y1[3,:])])
# end
# loopgain = Zgrid[3] .* getindex.(Y_Statcom[3][1][1], Ref(1:2), Ref(1:2))
# Zclosed=inv.(inv.(Zgrid[3]) + getindex.(Y_Statcom[3][1][1], Ref(1:2), Ref(1:2)))

# EVD(Zclosed, omegas, min_f, max_f)
...
#                         grid=IEEE39bus_parametric(topology, Q, V) 
                        
#                         # Check powerflow convergence
#                         if Int(PowerImpedanceACDC.result["termination_status"]) == 4 || Int(PowerImpedanceACDC.result["termination_status"]) == 10

#                         # Check whether statcom values are in relax_bounds
#                         bus=PowerImpedanceACDC.nodes2bus[Set([:Bus9d, :Bus9q])][2]
#                         vm=PowerImpedanceACDC.result["solution"]["bus"][string(bus)]["vm"]
#                                 if (vm > 0.90) && (vm < 1.1)
...
#                         else
#                                 println("Powerflow did not converge for Q-STATCOM=$Q & Vac-STATCOM=$V in line disconnection: $lines")
#                                 println(PowerImpedanceACDC.result["termination_status"])
#                                 rescue = true
#                                 break
#                         end
#                         if rescue == true
...
#         end
#         if rescue == true
#                 println("Stopping further simulations due to non-convergence issues.")
#                 break
#         end
