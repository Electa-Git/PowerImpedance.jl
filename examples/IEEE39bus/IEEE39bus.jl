...
STATCOM= tlc(Vᵈᶜ = Vdc_ST, Lᵣ = Lf_ST, Rᵣ = Rf_ST, 
        ...
        vac_supp = PI_control(Kₚ=5*(S_ST/1000),ω_f=1/0.5,ref=[Vm1*sqrt(2)*Vac_ref_ST]),
        timeDelay = 200e-6, #!!!!!
        padeOrderNum = 3,                    
        padeOrderDen = 3 
)
...
n_f=5000 # Number of frequency points in the

@time Zgrid, omegas=determine_impedance(IEEE39bus, input_pins=[:Bus9d,:Bus9q], output_pins=[:gndD, :gndQ], elim_elements=[:STATCOM],freq_range=(min_f, max_f,n_f))


writedlm("imp_TL_validation.csv",  Zgrid, ',')
