...
function run_nets(L_arg)
	...
	@info "Building network with L_arg = $L_arg"
...
L_range = [4.9916e-4, 6.4458e-4, 3.5523e-4]
imp_ac = Vector{Any}(undef, length(L_range))
omega_ac = Vector{Any}(undef, length(L_range))

for k ∈ 1:length(L_range)
	L_arg = L_range[k]
	imp_ac[k], omega_ac[k] = run_nets(L_arg)
end

k=3
testZ = getindex.(imp_ac[k], 1, 1)
bodeplot(testZ, omega_ac[k], legend = "L_arg=$(L_range[k])")
# imp_ac1, omega_ac1 = run_nets(L_range)

# testZ = getindex.(imp_ac1, 1, 1)
# bodeplot(testZ, omega_ac1, legend = "testZ")
